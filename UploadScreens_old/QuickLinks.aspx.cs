    ...
    public partial class QuickLinks : System.Web.UI.Page
    {
        ...
        protected void ButtonUpload_Click(object sender, EventArgs e)
        {
            ...
            if (FileUploadImages.HasFiles)
            {
                try
                {
                    string applicationName = TextBoxApplicationName.Text.Trim().ToLower();
                    string applicationURL = TextBoxApplicationURL.Text.Trim().ToUpper();
                    ...
                    else if (string.IsNullOrEmpty(applicationURL))
                    {
                        throw new Exception("Application URL cannot be empty.");
                    }

                    string appBasePath = Server.MapPath("~/");
                    // Create a folder for the project if it doesn't exist
                    string projectFolderPath = Path.Combine(appBasePath, "LandingPageResources/QuickLinks/");
                    ...
                    foreach (HttpPostedFile uploadedFile in FileUploadImages.PostedFiles)
                    {
                        ...
                        {
                            string query = @"INSERT INTO QuickLinksDetails (ApplicationName, ApplicationURL, IconPath, CreatedOn, CreatedBy, ModifiedOn, ModifiedBy)
                                           VALUES (@ApplicationName, @ApplicationURL, @IconPath, GETDATE(), @CreatedBy, GETDATE(), @ModifiedBy)";
                            SqlCommand cmd = new SqlCommand(query, con);
                            cmd.Parameters.AddWithValue("@ApplicationName", applicationName);
                            cmd.Parameters.AddWithValue("@ApplicationURL", applicationURL);
                            cmd.Parameters.AddWithValue("@IconPath", resolvedFilePath);
                            ...
                            TextBoxApplicationURL.Text = string.Empty;
                    ...
                    Response.Write("<script>alert('Links uploaded successfully!');</script>");
                }
                catch (Exception ex)
                {
                    // Display error message
                    Response.Write("<script>alert('Error uploading links: " + ex.Message + "');</script>");
                }
            }
            else
            {
